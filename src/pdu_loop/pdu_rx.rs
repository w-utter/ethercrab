...
impl<'sto> PduRx<'sto> {
    ...
    /// the io_uring version of `receive_frame`
    pub fn receive_frame_io_uring(&mut self, ethernet_frame: &[u8]) -> Result<Option<crate::pdu_loop::frame_element::receiving_frame::ReceivingFrame<'sto>>, Error> {
        let raw_packet = EthernetFrame::new_checked(ethernet_frame)?;
